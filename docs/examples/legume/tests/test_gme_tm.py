...
class TestGME_TM(unittest.TestCase):
    '''
    Test of single-layer legume vs. stored data from my MATLAB GME
    Lowest-order TM guided band included only
    '''
    def test_square(self):
        ...
        options = {'gmode_inds': [1], 'numeig': 10, 'verbose': False}
        gme.run(kpoints=np.array([[0., 0.1], [0., 0.2]]), **options)

        dat = scipy.io.loadmat('./tests/data/gme_square_tm.mat')
