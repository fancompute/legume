...
from legume.gme.slab_modes import guided_modes

g_array = np.linspace(0.1, 13, 30)
eps_array = np.array([1.0, 12, 1.0, 12, 1.0])
d_array = np.array([1.0, 0.1, 1.0])

(omegas_TE, _) = guided_modes(g_array, eps_array, d_array, n_modes=10, step=1e-2, tol=1e-4, pol='TE')
(omegas_TM, _) = guided_modes(g_array, eps_array, d_array, n_modes=10, step=1e-2, tol=1e-4, pol='TM')
